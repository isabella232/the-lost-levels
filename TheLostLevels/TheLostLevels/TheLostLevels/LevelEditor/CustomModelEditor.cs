...
namespace TheLostLevels
{
    ...
    /// </summary>
    public class CustomModelEditor : DrawableGameComponent
    {
        Model themodel;
        ...
        private LevelEditor thisGame;
        private float[] Properties;
        //constructor will eventually need file name of mesh model to be given as one of the inputs
        public CustomModelEditor(LevelEditor g, Vector3 position, Model ourmodel,float[] properties, string name)   
            : base(g)
        {
            Position =position;
            thisGame = g;
